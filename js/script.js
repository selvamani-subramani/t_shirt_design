...
tshirts.ajax_load_images = function() {
  $(".text-decimal .image-box").off("click").on("click", function() {
    ...
    tshirts.active_click_event();
    tshirts.load_image_to_box();
    tshirts.history_tracker();
  })
  //tshirts.load_image_to_box();
}

tshirts.history_tracker = function(){
...
tshirts.load_image_to_box = function() {
  $(".text-decimal .load-images").on("click", "img", function() {
    if (!$(this).hasClass("clipart")) {
      image_data = $(this).attr("src");
      $(".text-decimal .image-box img").attr("src", image_data);
      $(".text-decimal .load-images").removeClass("hidden");
      tshirts.update_image_and_text();
      tshirts.change_image_color($("#text_content img")[0]);
    }
  })
...
tshirts.active_click_event = function() {
  if (!$(".load-images").hasClass('hidden')) {
    $(".load-images").off('click').on('click', ".clipart", function() {
